...
export default function Cookbook() {
    ...
    // ✅ New effect: open the recipe automatically if coming from Navbar search
    useEffect(() => {
        const recipeId = location.state?.openRecipeId;
        if (recipeId) {
            handleRecipeSelect(recipeId);
            // Remove the state so refreshing doesn't reopen it
            window.history.replaceState({}, '', '/cookbook');
        }
    }, [location.state]);
